...
const roleLabel = (v) => {
  ...
  const r = roleOptions.value.find(i => i.value === key)
  return r ? r.label : (key ? key.charAt(0).toUpperCase() + key.slice(1) : '')
}
...
const loadModules = async () => {
  try {
    ...
    const list = (resp.data?.list || []).filter(m => m.code !== 'sys_module')
...
const openEditDialog = (row) => {
  ...
  Object.assign(dialog.form, {
    ...row,
    allowedRoles: Array.isArray(row.allowedRoles) ? row.allowedRoles.map(r => (r || '').toString().toLowerCase()) : []
  })
  dialog.visible = true
}
