...
router.beforeEach((to, from, next) => {
  ...
  const disabledModules = (localStorage.getItem('disabledModules') || '').split(',').filter(Boolean)
  const disabledReady = localStorage.getItem('disabledModulesReady') === '1'
  ...
  if (to.meta.requiresAuth && !isAuthenticated) {
    next({ name: 'Login' })
  } else if (disabledReady && to.meta.moduleCode && disabledModules.includes(to.meta.moduleCode)) {
    // 模块被禁用拦截
